...
# This script trains a fairly vanilla network with RELU  nonlinearities.
...
if [ $# != 4 ]; then
  ...
  echo "  --left-context <width|15>                         # Number of frames on left side to append for feature input"
  echo "  --right-context <width|5>                         # Number of frames on left side to append for feature input"
...
SpliceComponent input-dim=$ext_feat_dim left-context=$left_context right-context=$right_context
...
RectifiedLinearComponent dim=$hidden_layer_dim
