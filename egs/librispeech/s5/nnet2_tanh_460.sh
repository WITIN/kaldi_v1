...
#wsd6# 
#wsd6# 
#wsd6# if $use_gpu; then
#wsd6#   if ! cuda-compiled; then
#wsd6#     cat <<EOF && exit 1 
#wsd6# This script is intended to be used with GPUs but you have not compiled Kaldi with CUDA 
#wsd6# If you want to use GPUs (and have them), go to src/, and configure and make on a machine
#wsd6# where "nvcc" is installed.
#wsd6# EOF
#wsd6#   fi  
#wsd6#   parallel_opts="--gpu 1"
#wsd6#   num_threads=1
#wsd6#   minibatch_size=512
#wsd6#   dir=exp/tri5b_nnet2_tanh
#wsd6# else
#wsd6#   # with just 4 jobs this might be a little slow.
#wsd6#   num_threads=16
#wsd6#   parallel_opts="--num-threads $num_threads"
#wsd6#   minibatch_size=128
#wsd6#   dir=exp/tri5b_nnet2_tanh
#wsd6# fi
#wsd6# . utils/parse_options.sh
#wsd6# 
#wsd6# train_stage=-10
#wsd6# dnn_mem_reqs="--mem 8G"

#wsd6# #dnn_train_extra_opts="--num-epochs 20 --num-epochs-extra 5 --add-layers-period 1 --shrink-interval 3"
...
#wsd6#       --left-context 15 \
#wsd6#       --right-context 5 \
...
#wsd6# fi
#wsd6#   
#wsd6# for test in test_clean test_other dev_clean dev_other; do
#wsd6# #wsd#    --transform-dir exp/tri4b/decode_tgsmall_$test \
#wsd6#   steps/nnet2/decode.sh --nj 30 --cmd "$decode_cmd" --feat-type raw \
#wsd6#     exp/tri4b/graph_tgsmall data-fbank/$test $dir/decode_tgsmall_$test || exit 1;
#wsd6#   steps/lmrescore.sh --cmd "$decode_cmd" data/lang_test_{tgsmall,tgmed} \
#wsd6#     data-fbank/$test $dir/decode_{tgsmall,tgmed}_$test  || exit 1;
#wsd6#   steps/lmrescore_const_arpa.sh \
#wsd6#     --cmd "$decode_cmd" data/lang_test_{tgsmall,tglarge} \
#wsd6#     data-fbank/$test $dir/decode_{tgsmall,tglarge}_$test || exit 1;
#wsd6#   steps/lmrescore_const_arpa.sh \
#wsd6#     --cmd "$decode_cmd" data/lang_test_{tgsmall,fglarge} \
#wsd6#     data-fbank/$test $dir/decode_{tgsmall,fglarge}_$test || exit 1;
#wsd6# done

#wsd6# #tanh nnet2 training ends here

#wsd2# local/download_and_untar.sh $data $data_url train-clean
#wsd2# 
#wsd2# # now add the "clean-360" subset to the mix ...          
#wsd2# local/data_prep.sh \                                   wait
#wsd2#   $data/LibriSpeech/train-clean-360 data/train_clean_36
#wsd2# steps/make_mfcc.sh --cmd "$train_cmd" --nj 40 data/trai
#wsd2#   exp/make_mfcc/train_clean_360 $mfccdir
#wsd2# steps/compute_cmvn_stats.sh \
#wsd2#   data/train_clean_360 exp/make_mfcc/train_clean_360 $m
#wsd2# 
#wsd2# # ... and then combine the two sets into a 460 hour one
#wsd2# utils/combine_data.sh \
#wsd2#   data/train_clean_460 data/train_clean_100 data/train_
#wsd2# 
#wsd2# # align the new, combined set, using the tri4b model
#wsd2# steps/align_fmllr.sh --nj 40 --cmd "$train_cmd" \
#wsd2#   data/train_clean_460 data/lang exp/tri4b exp/tri4b_ali_clean_460
#wsd2# 
fbankdir=fbank
for part in train_clean_460; do
  cp -r data/$part data-fbank/$part
  steps/make_fbank.sh --cmd "$train_cmd" --nj 40 data-fbank/$part exp/make_fbank/$part $fbankdir
  steps/compute_cmvn_stats.sh data-fbank/$part exp/make_fbank/$part $fbankdir
done
#### <wsd fbank>> ############

#wsd2# # create a larger SAT model, trained on the 460 hours o
#wsd2# steps/train_sat.sh  --cmd "$train_cmd" 5000 100000 \
#wsd2#   data/train_clean_460 data/lang exp/tri4b_ali_clean_46
#wsd2# 
#wsd2# # decode using the tri5b model
#wsd2# (
#wsd2#   utils/mkgraph.sh data/lang_test_tgsmall \
#wsd2#     exp/tri5b exp/tri5b/graph_tgsmall
#wsd2#   for test in test_clean test_other dev_clean dev_other
#wsd2#     steps/decode_fmllr.sh --nj 20 --cmd "$decode_cmd" \
#wsd2#       exp/tri5b/graph_tgsmall data/$test \
#wsd2#       exp/tri5b/decode_tgsmall_$test
#wsd2#     steps/lmrescore.sh --cmd "$decode_cmd" data/lang_te
#wsd2#       data/$test exp/tri5b/decode_{tgsmall,tgmed}_$test
#wsd2#     steps/lmrescore_const_arpa.sh \
#wsd2#       --cmd "$decode_cmd" data/lang_test_{tgsmall,tglar
#wsd2#       data/$test exp/tri5b/decode_{tgsmall,tglarge}_$te
#wsd2#     steps/lmrescore_const_arpa.sh \
#wsd2#       --cmd "$decode_cmd" data/lang_test_{tgsmall,fglar
#wsd2#       data/$test exp/tri5b/decode_{tgsmall,fglarge}_$te
#wsd2#   done
#wsd2# )&
#wsd2# 
#wsd2# # train a NN model on the 460 hour set
#wsd2# local/nnet2/run_6a_clean_460.sh
#wsd2# 
######## 460h tanh stars below
# DNN hybrid system training parameters
...
EOF
  ...
  num_threads=1
  minibatch_size=512
  dir=exp/tri6a_460_nnet2_tanh
else
  # with just 4 jobs this might be a little slow.
  num_threads=16
  parallel_opts="--num-threads $num_threads"
  minibatch_size=128
  dir=exp/tri6a_nnet2_tanh_no_gpu
fi
. utils/parse_options.sh

train_stage=-10
dnn_mem_reqs="--mem 128G"
...
#      --splice-width 6 \
  steps/nnet2/train_tanh.sh \
      ...
      --cmd "$train_cmd" \
      --feat-type raw \
      --num-epochs 7 --num-epochs-extra 3 --add-layers-period 1 --shrink-interval 3 \
  data-fbank/train_clean_460 data/lang exp/tri5b $dir || exit 1
fi
...
#tanh nnet2 training ends here

#wsd2# local/download_and_untar.sh $data $data_url train-other
#wsd2# 
#wsd2# # prepare the 500 hour subset.
#wsd2# local/data_prep.sh \
#wsd2#   $data/LibriSpeech/train-other-500 data/train_other_50
#wsd2# steps/make_mfcc.sh --cmd "$train_cmd" --nj 40 data/trai
#wsd2#   exp/make_mfcc/train_other_500 $mfccdir
#wsd2# steps/compute_cmvn_stats.sh \
#wsd2#   data/train_other_500 exp/make_mfcc/train_other_500 $m
#wsd2# 
#wsd2# # combine all the data
#wsd2# utils/combine_data.sh \
#wsd2#   data/train_960 data/train_clean_460 data/train_other_
#wsd2# 
#wsd2# steps/align_fmllr.sh --nj 40 --cmd "$train_cmd" \
#wsd2#   data/train_960 data/lang exp/tri5b exp/tri5b_ali_960
